    ...
    <div class="btn-group">
        ...
        <div class="dropdown-menu dropdown-menu-left">
            <button class="dropdown-item" type="button">
                <a class="m-link m-link--state m-link--warning" href="{{url(config('app.frontend_url').'/tv/'.$gallery->gallery_slug)}}" target="_blank"><i class="fa fa-eye"> Preview</i></a>
            </button>
            @can('update-gallery', $gallery)
                <button class="dropdown-item" type="button">
                    <a class="m-link m-link--state m-link--info" href="{{action('\Modules\Gallery\Http\Controllers\VideoController@create').'?code='.encrypt($gallery->getKey())}}"><i class="fa fa-edit"> Edit</i></a>
                </button>
            @endcan
            @can('delete-gallery', $gallery)
                <form action="{{action('\Modules\Gallery\Http\Controllers\VideoController@destroy')}}" method="post" accept-charset="utf-8">
                    {{method_field('DELETE')}}
                    {{csrf_field()}}
