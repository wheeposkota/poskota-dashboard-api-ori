@extends('core::admin.'.$theme_cms->value.'.templates.parent')

@section('page_level_css')
    {{Html::style(module_asset_url('core:assets/metronic-v5/global/plugins/bootstrap-tagsinput/bootstrap-tagsinput.css'))}}
@endsection

@section('title_dashboard', ' Member Subscription E-Paper')

@section('breadcrumb')
        <ul class="m-subheader__breadcrumbs m-nav m-nav--inline">
            <li class="m-nav__item m-nav__item--home">
                <a href="#" class="m-nav__link m-nav__link--icon">
                    <i class="m-nav__link-icon la la-home"></i>
                </a>
            </li>
            <li class="m-nav__separator">-</li>
            <li class="m-nav__item">
                <a href="" class="m-nav__link">
                    <span class="m-nav__link-text">Home</span>
                </a>
            </li>
            <li class="m-nav__separator">-</li>
            <li class="m-nav__item">
                <a href="" class="m-nav__link">
                    <span class="m-nav__link-text">Member Subscription E-Paper</span>
                </a>
            </li>
        </ul>
@endsection

@section('content')

<div class="row">
    <div class="col-md-5">
        <div class="m-portlet m-portlet--tab m-portlet--skin-dark m-portlet--bordered-semi m--bg-brand">
            <div class="m-portlet__head">
                <div class="m-portlet__head-caption">
                    <div class="m-portlet__head-title">
                        <h3 class="m-portlet__head-text">
                            Detail by Click ID Number
                        </h3>
                    </div>
                </div>
            </div>
            <div class="m-portlet__body" id="subscription" data-subscription="{{action('\Modules\EPaper\Http\Controllers\SubscriptionController@getData')}}" v-cloak>
                <div class="m-widget13">
                    <div class="m-widget13__item">
                        <span class="m-widget13__desc m--align-right text-white">
                            Invoice Number :
                        </span>
                        <span class="m-widget13__text m-widget13__text-bolder text-white">
                            @{{ data.order_id != undefined ? data.order_id : '-' }}
                        </span>
                    </div>
                    <div class="m-widget13__item">
                        <span class="m-widget13__desc m--align-right text-white">
                            Member Name :
                        </span>
                        <span class="m-widget13__text m-widget13__text-bolder text-white">
                            @{{ data.name != undefined ? data.name : '-' }}
                        </span>
                    </div>
                    <div class="m-widget13__item">
                        <span class="m-widget13__desc m--align-right text-white">
                            Package Name :
                        </span>
                        <span class="m-widget13__text m-widget13__text-bolder text-white">
                            @{{ data.package_name != undefined ? data.package_name : '-' }}
                        </span>
                    </div>
                    <div class="m-widget13__item">
                        <span class="m-widget13__desc m--align-right text-white">
                            Total :
                        </span>
                        <span class="m-widget13__text m-widget13__text-bolder text-white">
                            @{{ data.trx_order_total != undefined ? 'Rp. '+data.trx_order_total : '-' }}
                        </span>
                    </div>
                    <div class="m-widget13__item">
                        <span class="m-widget13__desc m--align-right text-white">
                            Start Date :
                        </span>
                        <span class="m-widget13__text m-widget13__text-bolder text-white">
                            @{{ data.start_date != undefined ? data.start_date : '-' }}
                        </span>
                    </div>
                    <div class="m-widget13__item">
                        <span class="m-widget13__desc m--align-right text-white">
                            End Date :
                        </span>
                        <span class="m-widget13__text m-widget13__text-bolder text-white">
                            @{{ data.end_date != undefined ? data.end_date : '-' }}
                        </span>
                    </div>
                    <div class="m-widget13__item">
                        <span class="m-widget13__desc m--align-right text-white">
                            Status :
                        </span>
                        <span class="m-widget13__text m-widget13__text-bolder text-white">
                            @{{ data.trx_order != undefined ? data.trx_order.mst_ad_status.status_name : '-' }}
                        </span>
                    </div>
                    <div class="m-widget13__item">
                        <span class="m-widget13__desc m--align-right text-white">
                            Order Date :
                        </span>
                        <span class="m-widget13__text m-widget13__text-bolder text-white">
                            @{{ data.trx_order_date != undefined ? data.trx_order_date : '-' }}
                        </span>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="col-md-7">

        <!--begin::Portlet-->
        <div class="m-portlet m-portlet--tab">
            <div class="m-portlet__head">
                <div class="m-portlet__head-caption">
                    <div class="m-portlet__head-title">
                        <span class="m-portlet__head-icon m--hide">
                            <i class="fa fa-gear"></i>
                        </span>
                        <h3 class="m-portlet__head-text">
                            Master Data of Subscription
                        </h3>
                    </div>
                </div>
            </div>

            <div class="m-portlet__body">
                @if (!empty(session('global_message')))
                    <div class="col-md-8">
                        <div class="alert {{session('global_message')['status'] == 200 ? 'alert-info' : 'alert-warning' }} alert-dismissible fade show">
                            <button type="button" class="close" data-dismiss="alert" aria-label="Close"></button>
                            {{session('global_message')['message']}}
                        </div>
                    </div>
                @endif
                @if (count($errors) > 0)
                    <div class="col-md-8">
                        <div class="alert alert-danger">
                            <ul>
                                @foreach ($errors->all() as $error)
                                    <li>{{ $error }}</li>
                                @endforeach
                            </ul>
                        </div>
                    </div>
                @endif

                <!--begin: Datatable -->
                <table class="table table-striped responsive data-table-ajax" data-ajax="{{action('\Modules\EPaper\Http\Controllers\SubscriptionController@serviceMaster')}}" id="data-term" width="100%">
                    <thead>
                        <tr>
                            <th data-priority="1">ID</th>
                            <th>Member Name</th>
                            <th>Package Name</th>
                            <th>Total</th>
                            <th>Status</th>
                            <th>Created At</th>
                        </tr>
                    </thead>
                    <tbody>
                    </tbody>
                </table>

                <!--end: Datatable -->
            </div>


        </div>

        <!--end::Portlet-->

    </div>
</div>
{{-- End of Row --}}

@endsection

@section('page_level_js')
    {{Html::script(module_asset_url('core:assets/metronic-v5/global/plugins/bootstrap-tagsinput/bootstrap-tagsinput.min.js'))}}
    {{Html::script(module_asset_url('core:assets/js/autosize.min.js'))}}
    {{Html::script('vendor/laravel-filemanager/js/lfm.js')}}
    {{Html::script(module_asset_url('EPaper:resources/views/admin/'.$theme_cms->value.'/js/subscription.js').'?id='.filemtime(module_asset_path('EPaper:resources/views/admin/'.$theme_cms->value.'/js/subscription.js')))}}
@endsection